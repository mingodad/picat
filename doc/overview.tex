...
\end{verbatim}
An \texttt{if} statement\index{if statement} takes the form \texttt{if $Cond$ then $Goal_1$ else $Goal_2$ end}.\footnote{Picat also accepts Prolog-style if-then-else in the form \texttt{(If->Then;Else)} and mandates the presence of the else-part.} The \texttt{then} part can contain one or more \texttt{elseif} clauses. The \texttt{else} part can be omitted. In that case the else part is assumed to be \texttt{else true}. The built-in \texttt{throw $E$}\index{\texttt{throw/1}} throws term\index{term} $E$ as an exception\index{exception}.
...
In version 3.0 and later versions, Picat supports Prolog-style Horn clauses\index{Horn clause}\index{Prolog}. A Horn clause takes the form $Head$\ :-\ $Body$. When $Body$ is \texttt{true}, the clause can be written as $Head$. Let $Head$ be $p(t_1,\ldots,t_n)$. The Horn clause can be translated equivalently to the following pattern-matching rule:
\begin{tabbing}
aa \= aaa \= aaa \= aaa \= aaa \= aaa \= aaa \kill
\> $p(V_1,\ldots,V_n)$ \verb+?=>+\ $V_1 = \$t_1$, $\ldots$, $V_n = \$t_n$, $Body$.
\end{tabbing}
where the dollar symbols indicate that all of the $t_i$'s are terms, not function calls.
