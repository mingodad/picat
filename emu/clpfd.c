...
/* $comp_prop_fapp1(X,Y,N,Flag)  (X\=0, Y\=0)
   Bind Flag to:
   0 if either (X*Y#<0) or (abs(X-Y)#>=N)
   1 if (X*Y#<0) fails
   2 if (abs(X-Y)#>=N) fails
*/
int b_COMP_PROP_FAPP1(){
    BPLONG X,Y,N,Flag,minX,maxX,minY,maxY;
    BPLONG_PTR dv_ptr;

    X = FOLLOW(arreg+4); DEREF_NONVAR(X);
    Y = FOLLOW(arreg+3); DEREF_NONVAR(Y);
    N = FOLLOW(arreg+2); DEREF_NONVAR(N); N = INTVAL(N);
    Flag = FOLLOW(arreg+1); 
    ...
    if (maxX<0){
        if (maxY<0) return unify(Flag,BP_ONE); /* (X*Y#<0) fails */
        if (minY>0) return unify(Flag,BP_ZERO); /* (X*Y#<0) succeeds */
    } else if (minX>0) {
        if (maxY<0) return unify(Flag,BP_ZERO);
        if (minY>0) return unify(Flag,BP_ONE);
    }

    if ((minX-maxY>=N) || (minY-maxX>=N))
        return unify(Flag,BP_ZERO);
    if ((maxX-minY<N) && (maxY-minX<N))
        return unify(Flag,MAKEINT(2));
    return BP_TRUE;
}

/*
  $comp_prop_fapp2(X,Y,N,Flag) (X\=0 Y\=0)
  Bind Flag to:
  0 if either (X*Y#>0) or (abs(abs(X)-abs(Y))#>=N)
  01 if (X*Y#>0) fails because X<=0 Y>= 0
  10 if (X*Y#>0) fails because X>=0 Y<= 0
  2 if (abs(abs(X)-abs(Y))#>=N) fails
*/
int b_COMP_PROP_FAPP2(){
    BPLONG X,Y,N,Flag,minX,maxX,minY,maxY,t;
    BPLONG_PTR dv_ptr_x,dv_ptr_y;
    ...
    if (maxX<0){
        if (maxY<0) return unify(Flag,BP_ZERO);
        if (minY>0) return unify(Flag,MAKEINT(01L));
    } else if (minX>0) {
        if (minY>0) return unify(Flag,BP_ZERO);
        if (maxY<0) return unify(Flag,MAKEINT(10L));
    }
    /* continue to be suspended if minY<0 and maxY>0 */
    ...
    if (maxX<0){
        ...
        if (IS_IT_DOMAIN(dv_ptr_x)){
            ...
            minX = domain_next_bv(dv_ptr_x,0);
            t = domain_prev_bv(dv_ptr_x,0);
            if (-t<minX) minX = -t;
        }
    }
    if (maxY<0){
        ...
        if (IS_IT_DOMAIN(dv_ptr_y)){
            ...
            minY = domain_next_bv(dv_ptr_y,0);
            t = domain_prev_bv(dv_ptr_y,0);
            if (-t<minY) minY = -t;
        }
    }
    if ((minX-maxY>=N) || (minY-maxX>=N)) return unify(Flag,BP_ZERO);
    if ((maxX-minY<N) && (maxY-minX<N)) return unify(Flag,MAKEINT(2));
    return BP_TRUE;
}
